...
with st.form(key=f"url_form_{seed}", border=False):
    ...
    with col1:
        url_input = st.text_input("Atau tambahkan berkas dari URL:",
    ...
    if submit_url:
        ...
        if not url_input:
            ...
            with st.spinner("Memeriksa dan mengambil media dari URL..."):
                ...
                if error_msg:
                    st.toast(f"Gagal memproses URL: {error_msg}", icon="❌")
                elif files_from_url:
        ...
        try:
            if not is_video: # --- A. PROSES GAMBAR STATIS ---
                ...
                try:
                    ...
                    # Ganti semua separator `\` menjadi `/` untuk konsistensi lintas platform.
                    relative_original = os.path.relpath(original_path).replace("\\", "/")
                    relative_mask = os.path.relpath(archive_mask_dir).replace("\\", "/")
                    relative_overlay = os.path.relpath(overlay_path).replace("\\", "/") if overlay_path else None
