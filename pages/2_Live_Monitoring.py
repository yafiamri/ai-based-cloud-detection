...
if st.session_state.live["source_info"]:
    st.subheader("**🎬 Pratinjau Siaran Langsung**")
    _, col_video, _ = st.columns([1, 5, 1])
    with col_video:
        st.video(st.session_state.live["source_info"]["display_url"])
...
if "Manual" in st.session_state.live.get("roi_method", "Otomatis") and not is_config_disabled:
    with st.expander("✏️ Gambar ROI Manual", expanded=True):
        if st.session_state.live.get("preview_frame") is None:
            with st.spinner("Memuat pratinjau untuk kanvas ROI..."):
                # Menggunakan fungsi helper yang andal, bukan cv2.VideoCapture(URL)
...
# Mendefinisikan placeholder di luar loop utama agar persisten
info_placeholder = st.empty()
result_placeholder = st.empty()

if st.session_state.live.get("running"):
    # Tampilkan info bahwa monitoring berjalan
    ...
    # Menampilkan hasil terakhir (jika ada) sebelum loop dimulai
    if st.session_state.live.get("last_result"):
    ...
    while st.session_state.live.get("running"):
        ...
        # Lakukan proses pengambilan frame
        frame = get_frame_via_download(st.session_state.live["url_input"])
...
# Menampilkan hasil terakhir setelah monitoring berhenti
